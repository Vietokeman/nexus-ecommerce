...
const parseAvailableQuantity = (payload: unknown): number | null => {
  ...
  const candidate =
    result.availableQuantity ?? result.quantity ?? result.stockQuantity ?? result.availableStock;
...
export default function CheckoutPage() {
  ...
  const handleCreateOrder = async () => {
    if (reserveExpired) {
      toast.error(
        'Your 15-minute stock reservation has expired. Please refresh it before checkout.',
      );
      return;
    }
    ...
    try {
      ...
      try {
        await api.post(API_ENDPOINTS.ORDERS.CREATE, {
          ...
          totalPrice: total,
          shippingAddress: `${selectedAddress.street}, ${selectedAddress.city}, ${selectedAddress.state}, ${selectedAddress.country}`,
          invoiceAddress: `${selectedAddress.street}, ${selectedAddress.city}, ${selectedAddress.state}`,
        ...
        toast.warning('Order persistence fallback active: payment flow will continue.');
      }
          ...
          >
            ...
            >
              ...
              <Typography
                variant="h6"
                fontWeight={800}
                color={reserveExpired ? 'error.main' : 'success.main'}
              >
          ...
          >
            ...
            <Stack rowGap={3}>
              ...
              <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                ...
                <Chip
                  label={
                    selectedPaymentMethod === 'CARD' ? 'Card Payment selected' : 'Card Payment'
                  }
