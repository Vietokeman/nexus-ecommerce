...
```powershell
psql "host=localhost port=5436 dbname=NexusAiDb user=admin password=change_me_with_postgres_password" -c "SELECT extname FROM pg_extension WHERE extname='vector';"
```

Check tables:

```powershell
psql "host=localhost port=5436 dbname=NexusAiDb user=admin password=change_me_with_postgres_password" -c "\dt"
```
